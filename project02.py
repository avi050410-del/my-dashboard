...
img_b64 = get_image_base64("2fbe80.jpg") # וודא שזה שם הקובץ המדויק בגיטהאב
...
# כותרת
st.markdown(f"""<div style="{bg_style} padding: 40px; border-radius: 15px; text-align: center; color: white;"><h1>📊 דשבורד ניהול יועצים - מנהלת מרה"ס</h1></div>""", unsafe_allow_html=True)
...
st.bar_chart(df.groupby('תחום')[['מסגרת שעות', 'ניצול']].sum(), color=["#3498db", "#e74c3c"])
